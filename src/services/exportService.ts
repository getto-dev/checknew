...
export function generateStandaloneHTML(estimate: Estimate): string {
      ...
      const rowsHtml = section.items
        .map((item) => {
          const descHtml = item.description?.trim()
            ? `<div class="row-description">${escapeHtml(item.description.trim())}</div>`
            : '';
  ...
  const objectHtml = objectAddress
    ? `<span class="document-object">Объект: ${escapeHtml(objectAddress)}</span>`
    : '';

  const contractorHtml = estimate.companyName?.trim() || estimate.phone?.trim()
    ? `<div class="document-contractor">
        ${estimate.companyName?.trim() ? `<span class="contractor-name">${escapeHtml(estimate.companyName.trim())}</span>` : ''}
        ${estimate.phone?.trim() ? `<span class="contractor-phone">${escapeHtml(estimate.phone.trim())}</span>` : ''}
      </div>`
    : '';
  ...
  if (showSectionSummary) {
    summaryLines = `
      <div class="summary-row">
        <span>Работы:</span>
        <span>${escapeHtml(money(servicesSum))}</span>
      </div>
      <div class="summary-row">
        <span>Материалы:</span>
        <span>${escapeHtml(money(productsSum))}</span>
      </div>`;
  }
  ...
  if (discountAmount > 0) {
    discountHtml = `
      <div class="summary-row">
        ...
        <span>−${escapeHtml(money(discountAmount))}</span>
      </div>`;
  }

  const notesHtml = estimate.notes?.trim()
    ? `<div class="document-notes">
        <div class="notes-label">Примечания к смете:</div>
        <div class="notes-text">${escapeHtml(estimate.notes.trim())}</div>
      </div>`
    : '';
...
<title>${escapeHtml(documentTitle)}${objectAddress ? ` — ${escapeHtml(objectAddress)}` : ''}</title>
<style>
@page { size: A4; margin: 0; }
* { box-sizing: border-box; }
html, body { margin: 0; padding: 0; background: #fff; color: #23272b; }
body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; font-size: 8pt; }
.estimate-page { width: 595.28px; min-height: 841.89px; margin: 0 auto; padding: 34px; background: #fff; position: relative; }
.header-line { height: 2px; background: #2388c9; width: 100%; }
.document-title { margin-top: 15px; margin-bottom: 13px; min-height: 13px; display: flex; align-items: baseline; justify-content: space-between; gap: 18px; }
.document-title-left { display: flex; align-items: baseline; gap: 14px; overflow: hidden; }
.document-number { font-size: 10pt; line-height: 10pt; font-weight: 700; color: #23272b; white-space: nowrap; }
.document-object { font-size: 8pt; line-height: 10pt; color: #697078; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.document-contractor { font-size: 8pt; line-height: 10pt; color: #697078; text-align: right; white-space: nowrap; display: flex; gap: 8px; }
.contractor-name { font-weight: 600; color: #23272b; }
.section { margin: 0; }
.section-top-line { height: 1px; background: #2388c9; width: 100%; }
.table-header, .row { display: grid; grid-template-columns: 346px 55px 65px 61px; }
.table-header { height: 33px; align-items: start; padding-top: 13px; color: #697078; font-size: 8pt; line-height: 8pt; }
.table-header > :not(:first-child) { text-align: center; }
.row { min-height: 30px; align-items: center; border-bottom: 0.5px solid #d8dce0; }
.row-name { grid-column: 1; padding: 7px 12px 7px 0; font-size: 9pt; line-height: 11pt; color: #23272b; }
.row-description { margin-top: 2px; color: #697078; font-size: 7pt; line-height: 9pt; }
.row-values { grid-column: 2 / 5; display: grid; grid-template-columns: 55px 65px 61px; align-items: center; font-size: 8pt; line-height: 8pt; text-align: center; }
.row-values > div { text-align: center; color: #23272b; }
.summary { margin-top: 18px; margin-left: 326px; width: 201px; }
...
.grand-total-label { font-size: 10pt; line-height: 10pt; font-weight: 700; }
.grand-total-value { font-size: 13pt; line-height: 13pt; font-weight: 700; }
.document-notes { margin-top: 24px; padding-top: 12px; border-top: 0.5px solid #d8dce0; font-size: 7.5pt; color: #697078; line-height: 1.4; }
.notes-label { font-weight: 600; margin-bottom: 2px; color: #23272b; }
.top-bar { text-align: center; margin-bottom: 16px; }
.print-btn { display: inline-flex; align-items: center; gap: 6px; padding: 7px 16px; background: #2388c9; color: #fff; border: none; border-radius: 6px; font-size: 12px; font-weight: 600; cursor: pointer; }
.print-btn:hover { background: #1a74ad; }
@media print {
  body { print-color-adjust: exact; -webkit-print-color-adjust: exact; background: transparent; }
  .top-bar { display: none !important; }
  .estimate-page { margin: 0; box-shadow: none; width: 100%; min-height: auto; padding: 20mm; }
}
@media screen {
  body { padding: 24px 12px; background: #f0f2f5; display: flex; flex-direction: column; align-items: center; }
  .estimate-page { box-shadow: 0 4px 24px rgba(0,0,0,0.12); }
}
</style>
</head>
<body>
<div class="top-bar">
  <button class="print-btn" onclick="window.print()">Распечатать смету</button>
</div>
<main class="estimate-page">
  <div class="header-line"></div>
  <div class="document-title">
    <div class="document-title-left">
      <span class="document-number">${escapeHtml(documentTitle)}</span>
      ${objectHtml}
    </div>
    ${contractorHtml}
  </div>
  ...
  <div class="summary">
    ...
    <div class="grand-total">
      ...
      <span class="grand-total-value">${escapeHtml(money(grandTotal))}</span>
    </div>
  </div>
  ${notesHtml}
  <script type="application/json" id="__ESTIMATE_DATA__">${embeddedJson}</script>
  <script type="application/json" id="smeta-app-data">${embeddedJson}</script>
</main>
</body>
</html>`;
}
