...
export const CatalogBrowser: React.FC<CatalogBrowserProps> = ({
  ...
  const handleQtyChange = (itemId: string, direction: -1 | 1, step = 0.5) => {
    setItemQuantities((prev) => {
      const cur = prev[itemId] !== undefined ? prev[itemId] : 1;
      ...
      <div className="p-3.5 sm:p-4 border-b border-slate-800 bg-slate-900/95 backdrop-blur-xs space-y-3 flex-shrink-0">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Поиск по названию или синонимам (кран, труба, кабель...)"
            className="w-full rounded-xl bg-slate-950 border border-slate-700/80 pl-9 pr-8 py-2 text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition"
          />
          {searchQuery && (
            <button
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-2.5 min-h-[300px]">
        {filteredItems.length === 0 ? (
          ...
          filteredItems.map((item) => {
            const currentQty = itemQuantities[item.id] !== undefined ? itemQuantities[item.id] : 1;
            ...
            return (
              <div
                key={item.id}
                className={`rounded-xl border p-3 transition flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 ${
                  isJustAdded
                ...
                <div className="flex items-center justify-between sm:justify-end gap-3 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-800/60 flex-shrink-0">
                  ...
                  <div className="flex items-center gap-1.5">
                    <div className="flex items-center rounded-lg bg-slate-900 border border-slate-700 p-0.5">
                      <button
                        type="button"
                        onClick={() => handleQtyChange(item.id, -1, 0.5)}
                        title="Уменьшить"
                      ...
                      <input
                        ...
                        onChange={(e) => handleManualQtyInput(item.id, e.target.value)}
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition active:scale-95 cursor-pointer shadow-xs whitespace-nowrap ${
                        isJustAdded
