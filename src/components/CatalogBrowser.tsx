              ...
              <div key={item.id} className={`rounded-xl border p-3 transition flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 ${isJustAdded ? 'border-emerald-500/80 bg-emerald-500/10' : inEstimateQty > 0 ? 'bg-slate-800/80 border-slate-700/90 shadow-sm' : 'bg-slate-950/60 border-slate-800/80 hover:border-slate-700 hover:bg-slate-800/40'}`}>
                ...
                <div className="flex items-center justify-between sm:justify-end gap-3 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-800/60 flex-shrink-0">
                  <div className="text-left sm:text-right">
                    <div className="text-xs sm:text-sm font-bold text-amber-400 font-mono">{formatCurrency(item.price)}</div>
          ...
          <div className="w-full max-w-lg rounded-2xl bg-slate-900 border border-slate-800 p-5 sm:p-6 shadow-2xl text-slate-100 relative max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800 mb-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-amber-500/15 border border-amber-500/30 text-amber-400 flex items-center justify-center"><ListFilter className="w-5 h-5" /></div>
                <div><h2 className="text-lg font-bold text-white">Выберите раздел</h2><p className="text-xs text-slate-400">Фильтр позиций каталога</p></div>
              </div>
            ...
            <div className="space-y-2">
              ...
              {categories.map((category) => {
                ...
                return (
                  <button key={category} type="button" onClick={() => { setSelectedCategory(category); setIsCategoryModalOpen(false); }} className={`w-full flex items-center justify-between gap-3 rounded-xl border px-3.5 py-3 text-left transition ${isSelected ? 'bg-amber-500/15 border-amber-500/60 text-amber-300' : 'bg-slate-950 border-slate-700 text-slate-200 hover:bg-slate-800'}`}>
                    <div className="min-w-0"><div className="text-sm font-semibold truncate">{category}</div><div className="text-[11px] text-slate-400">Позиций: {count}</div></div>
